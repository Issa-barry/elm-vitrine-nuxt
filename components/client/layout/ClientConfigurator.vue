...
const selectedPreset = computed<PresetName>({
  get: () => config.value.preset,
  set: (value) => { config.value.preset = value; },
});

const selectedMenuMode = computed<"static" | "overlay">({
...
const updateColors = (type: "primary" | "surface", color: ColorOption) => {
  if (type === "primary") {
    config.value.primary = color.name;
    updatePreset(getPresetExtension());
    return;
  }

  config.value.surface = color.name;
  updateSurfacePalette(color.palette);
};

const onPresetChange = () => {
  const presetValue = presets[selectedPreset.value];
  const surfacePalette = surfaces.find((surface) => surface.name === config.value.surface)?.palette;
  ...
  <div class="config-panel absolute top-[3.25rem] right-0 w-64 p-4 bg-surface-0 dark:bg-surface-900 border border-surface rounded-border origin-top shadow-[0px_3px_5px_rgba(0,0,0,0.02),0px_0px_2px_rgba(0,0,0,0.05),0px_1px_4px_rgba(0,0,0,0.08)]">
    <div class="flex flex-col gap-4">
      <div>
        ...
        <div class="config-panel-colors">
          <button
            ...
            type="button"
            :title="color.name"
            :class="{ 'active-color': config.primary === color.name }"
      ...
      <div>
        ...
        <div class="config-panel-colors">
          <button
            ...
            type="button"
            :title="surface.name"
            :class="{ 'active-color': config.surface ? config.surface === surface.name : isDarkTheme ? surface.name === 'zinc' : surface.name === 'slate' }"
      ...
      <div class="config-panel-settings">
        <span class="config-panel-label">Preset</span>
        <SelectButton v-model="selectedPreset" :options="presetOptions" :allow-empty="false" @change="onPresetChange" />
      </div>

      <div class="config-panel-settings">
        <span class="config-panel-label">Mode du menu</span>
        <SelectButton v-model="selectedMenuMode" :options="menuModeOptions" option-label="label" option-value="value" :allow-empty="false" />
      </div>
    </div>
  </div>
</template>
