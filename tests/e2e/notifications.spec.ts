...
// migré vers le contrat paginé finalisé côté backend le 27/08/2026) : cloche
// desktop/tablette (ClientTopbar.vue, panneau ClientNotificationsPanel.vue)
...
// `resource` correspondant à un vrai item TEST_ACTIVITY) plutôt que de
// dépendre du mock backend réel. Les POST mark-read/mark-all-read restent
// réels, eux (mutent l'état MUTABLE de mock-backend.mjs, partagé pour toute
// la durée du run — un seul worker, un seul process) : depuis que ces deux
// endpoints renvoient un `unread_count` canonique (chantier backend du
// 27/08/2026), adopté tel quel côté Nuxt (useClientNotifications), L'ORDRE
// DES TESTS DE CE FICHIER IMPORTE — voir le commentaire devant le test
// "delivery.assigned exploitable" plus bas pour le détail exact.
const NOTIFICATIONS_FIXTURE = {
  ...
  meta: { current_page: 1, from: 1, last_page: 1, links: [], path: null, per_page: 20, to: 3, total: 3 },
  unread_count: 2,
};
...
test.describe("Espace client — centre de notifications", () => {
  ...
  test("desktop : ouvrir la cloche affiche les notifications réelles, non-lues mises en évidence", async ({ page }) => {
    ...
    await expect(panel.getByText("Versement reçu")).toBeVisible();
    await expect(panel.getByText(`${new Intl.NumberFormat("fr-FR").format(850_000)} GNF`)).toBeVisible();
  ...
  // Ordonné AVANT le test "Tout marquer comme lu" ci-dessous : les deux
  // appellent réellement leur POST respectif contre le mock backend
  // (state MUTABLE partagé pour toute la durée du run, voir le commentaire en
  // tête de fichier) — mark-all-read passerait tout le backend à lu:true,
  // ce qui changerait le `unread_count` canonique renvoyé par POST .../read
  // et casserait l'assertion "1 non lue" ci-dessous si ce test s'exécutait
  // après. mark-all-read reste, lui, correct quel que soit l'ordre (renvoie
  // toujours unread_count: 0).
  test("desktop : une notification \"delivery.assigned\" exploitable redirige vers Livraisons et ouvre le bon détail", async ({ page }) => {
    await mockNotifications(page);
    await loginAsTestUser(page);
    ...
    // Marquée lue au clic : POST .../read renvoie désormais un unread_count
    // canonique (chantier backend du 27/08/2026), adopté tel quel par
    // useClientNotifications::markRead — le badge passe de 2 à 1 non lue.
  ...
  test("desktop : \"Tout marquer comme lu\" vide le badge immédiatement (sans rechargement complet)", async ({ page }) => {
    ...
    // Même remarque que le test précédent.
    await Promise.all([
      page.waitForResponse((response) => response.url().includes("/api/client/notifications") && response.request().method() === "GET"),
      page.getByRole("button", { name: "Notifications, 2 non lues" }).click(),
    ]);
